...
export default function SignupAccount({
  ...
  const onUsernameBlur = (username: string) => {
    accountApi.validateUsername(username).then((response) => {
  ...
  const onPasswordBlur = (password: string) => {
    accountApi.validatePassword(password).then((response) => {
      let description = response.data.errMsg;
      if (response.data.valid) {
        description = '';
      }
        ...
        <form onSubmit={onNextBtnClick}>
          ...
          <CardContent>
            ...
            <div className="mt-2 flex flex-col space-y-1.5">
              ...
              />
              <Label className="text-red-600">
                {inputDescription.passwordConfirm}
              </Label>
            </div>
          </CardContent>
          <CardFooter>
